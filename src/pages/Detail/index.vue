        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="s in spuSaleAttrList" :key='s.id'>
                ...
                <dd 
                  ...
                  v-for="spu in s.spuSaleAttrValueList" 
                  :key="spu.id"
                  @click="changeAttr(spu,s.spuSaleAttrValueList)"
                  >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 加入购物车前先发送请求，存储数据 -->
                <a href="javascript:(0);" @click='addShopCar'>加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      skuImageList() {
        //如果服务器没有返回数据，至少要给一个空对象，不然会报错
        return this.skuInfo.skuImageList || [];
      },
    },
    methods: {
      //商品售卖属性切换
      changeAttr(saleAttrValue, saleAttrValueList) {
        //排他思想
        //遍历所有属性,干掉所有人
      ...
      async addShopCar() {
        ...
        try {
          //2.发送成功,进行路由跳转
          //带上商品型号 spuSaleAttrList
          let shopAttr = {};
          this.spuSaleAttrList.forEach(item => {
            item.spuSaleAttrValueList.forEach(i => {
              if (i.isChecked == 1) {
                shopAttr[i.saleAttrName] = i.saleAttrValueName
              }
            })
          })
          await this.$store.dispatch('Detail/addOrUpdateShopCart', { skuId: this.$route.params.goodsId, skuNum: this.shopNum });
          //通过会话存储的方式保存数据，在购物车组件里读取本地存储数据
          //因为会话存储只能存字符串格式，所以要把数据转成JSON的字符串格式
          sessionStorage.setItem('skuInfo', JSON.stringify(this.skuInfo))
          sessionStorage.setItem('goodsAttrs',JSON.stringify(shopAttr))
          //路由跳转
          this.$router.push({name:'addCarSuccess',params:{num:this.shopNum}})
        } catch (error) {
          //3.发送失败，给用户提示
          alert(error.message)
        }
